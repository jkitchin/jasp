;;; vasp-mode.el --- Minor-mode to work with VASP calculations
...
(let ((all-keywords (append vasp-keywords vasp-other-words)))
...
(defun vasp-ag ()
  "Run ag on the POSCAR file in current directory."
  (interactive)
  (shell-command "ase-gui POSCAR"))


(defun vasp-run ()
  "Run VASP in the current directory.
Assumes the vasp command is \"vasp\"."
  (interactive)
  (start-process-shell-command "vasp" "*scratch*" "vasp"))


;; TODO: figure out a way to get options to qsub?
(defun vasp-queue ()
  "Submits job to a queue system.
Assumes the job is to run vasp in the current directory."
  (interactive)
  (shell-command "echo \"pwd; vasp\" | qsub -d `pwd` -l walltime=12:00"))


(defun vasp-nval (chemical-symbol functional )
  "Return number of valence electrons for a CHEMICAL-SYMBOL.
The POTCAR path is constructed as $VASP_PP_PATH/potpaw_FUNCTIONAL/CHEMICAL-SYMBOL/POTCAR."
  (interactive "sChemical Symbol: \nsFunctional: ")
...
(defun vasp-insert-keyword (keyword)
  "Insert a vasp KEYWORD at point."
  (interactive (list (ido-completing-read "Keyword: " vasp-keywords)))
  (insert "%s = " keyword))
...
(defvar vasp-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c C-i") 'vasp-insert-keyword)
