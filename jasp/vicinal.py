...
def fcc_vicinal(symbol,a,hkl,size,vacuum=None):
    ...
    #this is the primitive fcc cell. all atoms are integer combinations of these
...
if __name__ == '__main__':
    ...
    if '-p' in sys.argv:
        ...
        print 's0 = np.array([%1.0f, %1.0f, %1.0f])' % tuple(np.dot(s0,np.linalg.inv(rcell)))
        print 's1 = np.array([%1.0f, %1.0f, %1.0f])' % tuple(np.dot(s1,np.linalg.inv(rcell)))
        print 'offset = np.array([%1.0f, %1.0f, %1.0f])' % tuple(np.dot(offset_vector,np.linalg.inv(rcell)))
